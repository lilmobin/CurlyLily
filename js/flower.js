...
export class Flower {
  ...
  draw(ctx, dt, t, bloom, dissolve, w, h) {
    ...
    ctx.globalCompositeOperation = 'screen';   // soft accumulation: overlapping petals never burn out
    ...
    if (cg > 0.01 && vis > .01) {
      ...
      const R = s * (.34 + cg * .50) * pulse * (1 + dis * .25);
      const gx = this.cx, gy = this.cy - s * .28;
      const g = ctx.createRadialGradient(gx, gy, 0, gx, gy, R);
      g.addColorStop(0,   `rgba(255,200,216,${.15 * cg * vis})`);
      g.addColorStop(.30, `rgba(255,164,192,${.075 * cg * vis})`);
      g.addColorStop(.70, `rgba(255,140,172,${.018 * cg * vis})`);
    ...
    for (const layer of order) {
      for (const p of this.petals) {
        ...
        if (fill > .01 && pv > .005) {
          ctx.fillStyle = `rgba(250,176,202,${(.135 * fill * vis * dim * pv).toFixed(3)})`;
          ctx.beginPath();
        ...
        strokePass(2.2, `rgba(255,176,202,${.065 * oa})`);
        strokePass(.8, `rgba(255,222,232,${.54 * oa})`);

        /* veins brighten as the petal opens */
        const veinA = easeOutCubic(seg(grow, .55, 1)) * seg(bloom, .55, .95);
        if (veinA > .01) {
          ctx.lineWidth = .6;
          ctx.strokeStyle = `rgba(255,226,236,${.42 * veinA * oa})`;
          ctx.beginPath();
    ...
    if (base > .01) {
      ...
      g.addColorStop(0, `rgba(255,200,216,${.055 * base})`);
  ...
  _motes(ctx, dt, t, bloom, dis, w, h) {
    ...
    for (const m of this.motes) {
      ...
      if (dis <= 0) {
        ...
        ctx.globalAlpha = clamp(near) * .55;
        const sz = m.size * (k < 1 ? 5.5 : 4.8);
        ...
        ctx.globalAlpha = clamp(tw * fade * .62);
        const sz = m.size * (4.6 + tw * 1.6);
