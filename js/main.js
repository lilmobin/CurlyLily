...
function render(T, dt, clock) {
  ...
  /* ── ABSOLUTE BLACK first, then the scene wakes up ──
        black: 1 (pure #000, nothing visible) for the first beat → 0 as the red lines enter.
        dark : the wine-dark scene that then lifts into dusty pink (the "pink glow expanding"). */
  const black = 1 - easeInOutSine(seg(T, .5, 2.6));
  stage.style.setProperty('--black', black.toFixed(3));
  const dark = 1 - easeInOutSine(seg(T, 1.2, K.loadStart + .4));
  stage.style.setProperty('--dark', dark.toFixed(3));
  const pink = 1 - dark;                                       // how far the pink light has spread

  /* nebula haze: builds with the pink glow, peaks while loading, settles calm on the final screen */
  const haze = pink * (1 - .65 * easeInOutSine(seg(T, K.loadEnd, K.dissolveEnd + 1)));
  stage.style.setProperty('--haze', haze.toFixed(3));

  /* line colour: deep crimson at first; turns white ONLY after the pink light has reached the scene
        (pink .22 → .88  ≈  2.8s → 5.1s on the default timeline) */
  const lineWhite = easeInOutSine(seg(pink, .22, .88));

  /* ── AMBIENT particles fade in after the first beat ── */
  const ambient = easeOutCubic(seg(T, .6, 2.0));
  ...
  const bloomSwell = .2 * easeInOutSine(seg(T, K.loadStart, K.loadEnd)) * (1 - seg(T, K.dissolveStart, K.dissolveStart + 1.5));
  const finalWhisper = .12;
  const trailAlpha = Math.max(finalWhisper, trailAlphaMain + bloomSwell);

  /* ── draw scene layers ── */
  trails.draw(ctx, dt, clock, W, H, { draw: introTrailDraw, alpha: trailAlpha, white: lineWhite });
  ...
  /* mobile browser bar: black while the screen is black, then the top-of-scene rose */
  if (!themeSet && T > 2.6) { themeSet = true; document.querySelector('meta[name="theme-color"]')?.setAttribute('content', '#8E424E'); }
